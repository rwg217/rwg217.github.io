turtles-own
  [ zombie?                ;; if true, the turtle is is dead
    remaining-immunity   ;; how many weeks of immunity the turtle has left
    zombie-time            ;; how long, in weeks, the turtle has been is dead
...
to get-zombie ;; turtle procedure
  set zombie? true
  set remaining-immunity 0
end
...
to setup-constants
  set lifespan 10 * 52      ;; 10 times 52 weeks = 10 years = 520 weeks old
  set carrying-capacity 300
  set chance-reproduce 1
  set immunity-duration 25
end
...
to go
  ask turtles [
    get-older
    move
    if zombie? [ recover-or-die ]
    ifelse zombie? [ infect ] [ reproduce ]
  ]
  update-global-variables
  update-display
  tick
...
to update-global-variables
  if count turtles > 0
    [ set %infected (count turtles with [ zombie? ] / count turtles) * 100
...
to update-display
  ask turtles
    [ if shape != character
      [ set shape character ]
      set color ifelse-value zombie? [ red ] [ ifelse-value immune? [ blue ] [ green ] ] ]
end
...
to get-older ;; turtle procedure
  ...
  if zombie? [ set zombie-time zombie-time + 1 ]
end
...
;; If a turtle is zombie, it infects other turtles on the same patch.
;; Immune turtles don't get zombie.
to infect ;; turtle procedure
  ask other turtles-here with [ not zombie? and not immune? ]
    [ if random-float 100 < transmissionrate
      [ get-zombie ] ]
end

;; Once the turtle has been zombie long enough, it
;; either recovers (and becomes immune) or it dies.
to recover-or-die ;; turtle procedure
  if zombie-time > duration                        ;; If the turtle has survived past the virus' duration, then
    [ ifelse random-float 100 < 0 ;;chance-recover   ;; either recover or die
      [ become-immune ]
      [ die ] ]
end
